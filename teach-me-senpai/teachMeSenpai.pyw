...
class gamef:
    ...
    def grid():
        ty = False
        gameImage = pygame.image.load("grass.jpg")
        for x in range(int(vr.sw/vr.pxl)):
            for y in range(int(vr.sh/vr.pxl)):
                ...
                if (y%2)==0:
                    of = 1
                if (((x+of)%2)==0):
                    # pygame.draw.rect(screen, (255,255,255),pygame.Rect(x*vr.pxl,y*vr.pxl,vr.pxl,vr.pxl))
                    screen.blit(gameImage,(x*vr.pxl,y*vr.pxl))
    ...
    def draw():
        ...
        gameImage = pygame.image.load("teachmesenpai.png")
        # gameImage = gameImage.resize((vr.sw, vr.pxl), PIL.Image.ANTIALIAS)
        screen.blit(gameImage,(40, 5))
        # bottom
        ...
        bombImage = pygame.image.load("car.png")
        screen.blit(bombImage,((vr.pxl*bomb.x+4,vr.pxl*bomb.y+3, vr.pxl-4, vr.pxl-4)))
        # apple colour
        color=(0,255,0)
        # pygame.draw.rect(screen, color, pygame.Rect(vr.pxl*apple.x+2,vr.pxl*apple.y+2, vr.pxl-4, vr.pxl-4))
        birdImage = pygame.image.load("birdbird.png")
        screen.blit(birdImage,((vr.pxl*apple.x+2,vr.pxl*apple.y+3, vr.pxl-4, vr.pxl-4)))
        os = vr.coloroffset
        # colour snaketail
    ...
    def retrieve_UDP_values():
        ...
        try:
            ...
            A_Y = "%1.4f" %unpack_from ('!f', data, 40)
            A_Z = "%1.4f" %unpack_from ('!f', data, 44)
            A_Z = int(float(A_Z))
            A_Y = int(float(A_Y))
...
while not vr.done:
    ...
    if (c >= (100/snake.speed)):
        ...
        #snake.speed+=1
    if not(snake.dire  == 5):
        pygame.display.set_caption("Teach Me Senpai!!!    Score: "+str(snake.leng-4))
    pygame.display.flip()
    clock.tick(20)
